...
export async function fetchGitHubActivity(token: string): Promise<RepoActivity[]> {
  ...
  const variables = {
    ...
    to: now.toISOString(),
    issueSearchQuery: `is:issue involves:${SITE.githubUsername} updated:>${start.toISOString().split('T')[0]}`,
    mergedPRSearchQuery: `is:pr is:merged user:${SITE.githubUsername} -author:${SITE.githubUsername} -author:app/dependabot -author:app/renovate merged:>${start.toISOString().split('T')[0]}`
  }
  ...
  try {
    ...
    return aggregateActivityByRepository(contributionsCollection, issueSearch, mergedPRSearch)
  } catch (error) {
...
function aggregateActivityByRepository(contributions: ContributionsCollection, issueSearch?: IssueSearchResponse['search'], mergedPRSearch?: MergedPRSearchResponse['mergedPRs']): RepoActivity[] {
  ...
  contributions.pullRequestReviewContributionsByRepository.forEach(repoContrib => {
    ...
    const validReviews = repoContrib.contributions.nodes.filter(contrib =>
      contrib.pullRequest.author.login !== SITE.githubUsername &&
  ...
  if (mergedPRSearch) {
    mergedPRSearch.nodes.forEach(pr => {
      ...
      if (!pr.merged || pr.mergedBy.login !== SITE.githubUsername) return;
      
      // Skip if authored by the user (already counted in PR contributions)
      if (pr.author.login === SITE.githubUsername) return;
      
      // Skip forked repositories
      if (pr.repository.isFork) return;
      
      // Only count merges for repos owned by the user
      if (pr.repository.owner.login !== SITE.githubUsername) return;
