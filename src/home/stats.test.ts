...
describe("queryCodeTotals", () => {
  it("counts rows by when their year was last active", async () => {
    ...
    expect(totals.month).toEqual({ prCount: null, repoCount: 1 });
    expect(totals.year).toEqual({ prCount: 5, repoCount: 2 });
    expect(totals.all).toEqual({ prCount: 14, repoCount: 2 });
  });

  it("is zeros with nothing synced", async () => {
    const totals = await queryCodeTotals(db, NOW);
    expect(totals.year).toEqual({ prCount: 0, repoCount: 0 });
