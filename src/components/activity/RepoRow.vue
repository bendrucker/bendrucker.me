...
<template>
  ...
  >
    ...
    <time
      ...
      :title="full"
      class="order-2 w-16 shrink-0 text-right text-xs text-foreground/50 sm:order-3"
    >
      {{ when }}
    </time>
    <span
      v-if="repo.description"
      class="order-3 min-w-0 basis-full truncate text-xs text-foreground/55 sm:order-2 sm:flex-1 sm:basis-0 sm:text-sm"
    >
      {{ repo.description }}
    </span>
  </div>
