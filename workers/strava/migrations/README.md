...
Before running migrations, create the D1 databases:
...
After creating the databases, update the `database_id` and `preview_database_id` fields in `wrangler.toml` with the IDs returned by the commands above.
